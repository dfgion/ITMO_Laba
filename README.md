...
> P.S. Это не СУБД!!1 Базы данных оптимизированы под хранение данных (имеют индексы для удобного взаимодействия)
...
>>Functions - каталог, включающий в себя python файлы, в которых вы будете писать логику взаимодействия. Он включает в себя 2 основных файла: database_func и bans_func

Для самых интересующихся, которым приглянулся фреймворк - напишу информацию о том, как писать приложения в конце.
...
## Основное задание
Если вы прочитали информацию выше - я вас поздравляю, вы молодец (я бы не читал). Остальные пропустили много пэйлоада для мозга. Итак, приступим к описанию вашего задания. Как вы можете увидеть у себя на экране - вы прошли процесс аутентификации(я на это надеюсь..), попали на главный экран, где есть 3 основных страницы. Home, Database и Bans. Пройдя по ним, вы заметите, что информация имеется только на странице Home, а вот остальные 2 почему-то пустые. Как вы можете уже догадаться - вашей задачей будет написание некоторых функций для того, чтобы информация появилась на страницах, а вместе с этим появилось и взаимодействие с этой информацией.
### Задание 1. DatabasePage
...
Полностью делать всю работу асинхронным кодом я вас не заставляю, но простые функции для понимая асинхронности в python всё же придётся сделать. Главная идея в том, чтобы вы поняли хотя бы примерно, как работает асинхронное программирование. И что такое IO-bound и CPU-bound нагрузки.
